...
def test_getspec_cmd_output():  
    ...
    (res_dict, output_txt) = simulate([__file__, 'getspec-command2'])
...
def test_config_cli_options():
    ...
    eq_(config['getspec']['foo'], 'some value')

    # Then overridden by cli_opts
    simulate([__file__, 'getspec', 'getspec-sub-command', '--foo=bar'])
    
    # now we validate that the config option was set by the cli_opt --foo
    eq_(config['getspec']['foo'], 'bar')
