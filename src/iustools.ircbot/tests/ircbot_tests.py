...
def test_ircbot_cmd_output():  
    ...
    (res_dict, output_txt) = simulate([__file__, 'ircbot-command2'])
...
def test_config_cli_options():
    ...
    eq_(config['ircbot']['foo'], 'some value')

    # Then overridden by cli_opts
    simulate([__file__, 'ircbot', 'ircbot-sub-command', '--foo=bar'])
    
    # now we validate that the config option was set by the cli_opt --foo
    eq_(config['ircbot']['foo'], 'bar')
