...
def test_admin_cmd_output():  
    ...
    (res_dict, output_txt) = simulate([__file__, 'admin-command2'])
...
def test_config_cli_options():
    ...
    eq_(config['admin']['foo'], 'some value')

    # Then overridden by cli_opts
    simulate([__file__, 'admin', 'admin-sub-command', '--foo=bar'])
    
    # now we validate that the config option was set by the cli_opt --foo
    eq_(config['admin']['foo'], 'bar')
